...
export function createDashboardRouter(services = {}) {
  const router = Router()
  const generate = services.generateDashboard || generateDashboard
  const refine = services.refineDashboard || refineDashboard
  ...
  router.post('/generate', async (req, res) => {
    ...
    try {
      const dashboard = await generate(description.trim())
      res.json({ dashboard })
    } catch (err) {
  ...
  router.post('/refine', async (req, res) => {
    ...
    try {
      const dashboard = await refine(currentConfig, instruction)
      res.json({ dashboard })
    } catch (err) {
